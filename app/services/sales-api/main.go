...
/*
	Need to figure it out timeouts
*/
...
func main() {
	...
	//Perform the start and shutdown sequence
...
func run(log *zap.SugaredLogger) error {
	...
	// Set the correct number of thread for the service
	// based on what is available either by the machine or quotas.

	if _, err := maxprocs.Set(); err != nil {
		return fmt.Errorf("maxprocs %w", err)
	}
	log.Infow("startup", "MAXPROCS", runtime.GOMAXPROCS(0))
	...
	if err != nil {
		...
		return fmt.Errorf("passing conf: %w", err)
	}
	...
	go func() {
		...
		serverErrors <- api.ListenAndServe() //blocks unless an error comes in
	}()
